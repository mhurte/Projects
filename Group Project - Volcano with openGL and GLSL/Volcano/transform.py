...
def camera_offset_from_mouse_pos(mouse_pos, distance):
    x = np.cos(mouse_pos[0])*np.cos(mouse_pos[1])
    y = np.sin(mouse_pos[1])
    z = np.sin(mouse_pos[0])*np.cos(mouse_pos[1])
...
class Trackball:
    ...
    def __init__(self, distance=6., min_distance = 0.8, min_height = 1., radians=None):
        ...
        #self.pos3d = vec(0,0,-self.distance)
        self.orientation = vec(np.sin(self.theta)*np.cos(self.phi),
                                np.cos(self.theta),
                                np.sin(self.theta)*np.sin(self.phi))
        #self.pos3d = -self.distance * vec(np.sin(self.theta)*np.cos(self.phi),
        #                        np.cos(self.theta),
        #                        np.sin(self.theta)*np.sin(self.phi))
    ...
    def drag(self, old, new, winsize):
        ...
        diff = new - old
        theta = self.theta + -diff[1]
        phi = self.phi + diff[0]
        if (-self.distance * vec(np.sin(theta)*np.cos(phi),
                                          np.cos(theta),
                                          np.sin(theta)*np.sin(phi)))[1] > self.min_height:
            self.theta = theta
            self.phi = phi
            self.orientation = vec(np.sin(self.theta)*np.cos(self.phi),
                                            np.cos(self.theta),
                                            np.sin(self.theta)*np.sin(self.phi))
        # Use to negate the y mouse movement component
        #old, new = ((2*vec(pos[0],0.5 * winsize[1]) - winsize) / winsize for pos in (old, new))
    ...
    def zoom(self, delta, size):
        ...
        if (-self.distance * (1 - 50*delta/size) * self.orientation + self.offset)[1] > self.min_height:
            self.distance = max(0.001, self.distance * (1 - 50*delta/size))

    def up(self):
        return -vec(np.cos(self.theta)*np.cos(self.phi), -np.sin(self.theta), np.cos(self.theta) * np.sin(self.phi))
    
    def position(self):
        return - self.distance * self.orientation + self.offset

    def pan(self, old, new):
        """ Pan in camera's reference by a 2d vector factor of (new - old) """
        diff = vec(new) - vec(old)
        if (self.position() - 0.1 * diff[1])[1] > self.min_height:
            self.offset -= vec(0,1,0) * 0.1 * diff[1]
        self.offset -= vec(1,0,0) * 0.1 * diff[0]
        #self.pos2d += (vec(new) - old) * 0.001 * self.distance

    def view_matrix(self):
        """ View matrix transformation, including distance to target point """
        return lookat(self.position(), self.offset, self.up())
    ...
    def matrix(self):
        ...
        return lookat(vec(0,0,0),self.orientation,self.up())
